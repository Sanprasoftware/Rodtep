...
class DutyDrawBackClaim(Document):
	def validate(self):
		...
		if self.round_off_amount >= round(flt(20,2)):
			frappe.throw("round of amount should be less than 20")
...
def journal_entry_list(start_date,end_date,company):
	...
	je_data = frappe.db.sql(f""" 
		select je.name as je_no, jea.debit_in_account_currency as debit_amount , je.cheque_date, je.cheque_no, si.shipping_bill_number as shipping_bill_no, c.custom_duty_drawback_receivable_account as account
...
def create_jv_on_submit(self,method):
	if(round(flt(self.total_debit_amount),4) == round(flt(self.script_amount),4)):
		meis_receivable_account = frappe.db.get_value("Company", { "company_name": self.company}, "custom_duty_drawback_receivable_account")
		meis_income_account = frappe.db.get_value("Company", { "company_name": self.company}, "custom_duty_drawback_income_account")
		meis_cost_center = frappe.db.get_value("Company", { "company_name": self.company}, "custom_duty_drawback_cost_center")
